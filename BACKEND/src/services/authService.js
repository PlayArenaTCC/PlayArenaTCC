...
async function assertCredentialsAreNotBanned({ email, cpf }) {
  const conditions = [];
  ...
  if (normalizedCpf) {
    conditions.push({ cpf: normalizedCpf });
  }
  ...
  const banned = await CredencialBanida.findOne({
    where: {
      [Op.or]: conditions,
    },
  });
  ...
  if (banned) {
    throw new HttpError(403, 'Estas credenciais foram bloqueadas permanentemente pela administração.');
  }
}
...
function normalizePhone(telefone, { required = false } = {}) {
  ...
  if (![10, 11].includes(digits.length)) {
    throw new HttpError(400, 'Telefone deve ter DDD e 8 ou 9 dígitos.');
  }
...
async function buildUserRegistration({ nome, cpf, email, senha, confirmar_senha, telefone }) {
  ...
  if (!validatePasswordConfirmation(senha, confirmar_senha)) {
    throw new HttpError(400, 'As senhas não conferem.');
  }

  await assertCredentialsAreNotBanned({
  ...
  if (existingUsuario?.email === normalizedEmail) {
    throw new HttpError(409, 'Este e-mail já está cadastrado como usuário.');
  }

  if (existingUsuario?.cpf === cpfNormalizado) {
    throw new HttpError(409, 'Este CPF já está cadastrado como usuário.');
  }

  if (existingProprietario?.email === normalizedEmail) {
    throw new HttpError(409, 'Este e-mail já está cadastrado como proprietário. Use outro e-mail ou faça login.');
  }

  if (existingProprietario?.cpf_cnpj === cpfNormalizado) {
    throw new HttpError(409, 'Este CPF já está cadastrado como proprietário.');
  }

  if (existingAdministrador?.email === normalizedEmail) {
    throw new HttpError(409, 'Este e-mail já está cadastrado como administrador.');
  }

  if (existingAdministrador?.cpf === cpfNormalizado) {
    throw new HttpError(409, 'Este CPF já está cadastrado como administrador.');
  }
...
async function buildOwnerRegistration({
  ...
  const documentoNormalizado = onlyDigits(cpf_cnpj);
  const normalizedEmail = ensureValidEmail(email);

  if (!String(nome_responsavel || '').trim()) {
    throw new HttpError(400, 'Informe o nome do responsável.');
  }

  const documentoError = getCpfCnpjError(cpf_cnpj);

  if (documentoError) {
    throw new HttpError(400, documentoError);
  }
  ...
  if (existingProprietario?.email === normalizedEmail) {
    throw new HttpError(409, 'Este e-mail já está cadastrado como proprietário.');
  }

  if (existingProprietario?.cpf_cnpj === documentoNormalizado) {
    throw new HttpError(409, 'Este CPF ou CNPJ já está cadastrado como proprietário.');
  }

  if (existingUsuario?.email === normalizedEmail) {
    throw new HttpError(409, 'Este e-mail já está cadastrado como usuário. Use outro e-mail ou faça login.');
  }

  if (existingUsuario?.cpf === documentoNormalizado) {
    throw new HttpError(409, 'Este CPF já está cadastrado como usuário.');
  }
  ...
  if (existingAdministrador?.cpf === documentoNormalizado) {
    throw new HttpError(409, 'Este CPF já está cadastrado como administrador.');
  }
...
async function createAdminAccount({
  ...
  try {
    ...
    if (error.name === 'SequelizeUniqueConstraintError') {
      throw new HttpError(409, 'Este e-mail ou CPF já está cadastrado.');
    }
...
function buildPendingRegistrationResponse(cadastroPendente, emailProvider = 'local') {
  ...
  return {
    ...
    message: isLocalEmail
      ? 'Código gerado em modo teste. Veja o terminal do backend.'
...
function buildEmailDeliveryError(error) {
  ...
  if (message.toLowerCase().includes('domain')) {
    return 'Não foi possível enviar o e-mail. O remetente de teste onboarding@resend.dev só envia para o e-mail da sua conta Resend. Para enviar para outros e-mails, verifique um domínio no Resend e use RESEND_FROM_EMAIL=PlayArena <no-reply@seudominio.com>.';
  }
  ...
  if (
    ...
    return 'Não foi possível enviar o e-mail pelo Gmail. Verifique se EMAIL_PROVIDER=gmail, se GMAIL_USER está correto e se GMAIL_APP_PASSWORD é uma senha de app de 16 caracteres gerada com a verificação em duas etapas ativada.';
  }

  return `Não foi possível enviar o e-mail de verificação. ${message}`;
}
...
async function resendRegistrationCode({ verification_id }) {
  ...
  if (!cadastroPendente) {
    throw new HttpError(404, 'Cadastro pendente não encontrado. Refaça o cadastro para receber um novo e-mail.');
  }

  if (cadastroPendente.expira_em < new Date()) {
    await cadastroPendente.destroy();
    throw new HttpError(410, 'Código expirado. Refaça o cadastro para receber um novo e-mail.');
  }
  ...
  if (cadastroPendente.reenvios >= maxResends) {
    await cadastroPendente.destroy();
    throw new HttpError(429, 'Limite de reenvios atingido. Refaça o cadastro para receber outro código por e-mail.');
  }
  ...
  if (cooldownSeconds > 0 && waitSeconds > 0) {
    throw new HttpError(429, `Aguarde ${waitSeconds} segundos para reenviar o código.`);
  }
...
function buildPasswordResetResponse(recuperacaoSenha, emailProvider = 'local') {
  ...
  return {
    ...
    message: emailProvider === 'local'
      ? 'Código de recuperação gerado em modo teste. Veja o terminal do backend.'
...
async function requestPasswordReset({ email }) {
  ...
  await clearExpiredTemporaryAccountBlock(found.account);

  if (isAccountTemporarilyBlocked(found.account)) {
    throw new HttpError(403, formatTemporaryBlockMessage(found.account));
  }

  if (found.perfil === 'usuario' && found.account.status !== 'ativo') {
    throw new HttpError(403, 'Usuário inativo. Não é possível recuperar a senha desta conta.');
  }

  if (found.perfil === 'proprietario' && found.account.ativo === false) {
    throw new HttpError(403, 'Conta inativa. Não é possível recuperar a senha desta conta.');
  }
...
async function assertPasswordResetCode(recuperacaoSenha, codigo) {
  if (!recuperacaoSenha) {
    throw new HttpError(404, 'Solicitação de recuperação não encontrada. Peça um novo código.');
  }

  if (recuperacaoSenha.expira_em < new Date()) {
    await recuperacaoSenha.destroy();
    throw new HttpError(410, 'Código expirado. Solicite um novo código de recuperação.');
  }

  const normalizedCode = onlyDigits(codigo);

  if (!/^\d{4,10}$/.test(normalizedCode)) {
    throw new HttpError(400, 'Informe o código numérico recebido por e-mail.');
  }
  ...
  if (recuperacaoSenha.tentativas >= maxAttempts) {
    await recuperacaoSenha.destroy();
    throw new HttpError(429, 'Limite de tentativas atingido. Solicite outro código.');
  }
  ...
  if (!codeMatches) {
    ...
    if (nextAttempts >= maxAttempts) {
      await recuperacaoSenha.destroy();
      throw new HttpError(429, 'Código inválido. Limite de tentativas atingido.');
    }

    await recuperacaoSenha.update({ tentativas: nextAttempts });
    throw new HttpError(400, 'Código inválido. Verifique o e-mail e tente novamente.');
  }
...
async function verifyPasswordResetCode({ reset_id, codigo }) {
  ...
  return {
    reset_id: recuperacaoSenha.id,
    email: maskEmail(recuperacaoSenha.email),
    message: 'Código validado. Defina sua nova senha.',
  };
}
...
async function resetPassword({ reset_id, codigo, senha, confirmar_senha }) {
  ...
  if (!account) {
    await recuperacaoSenha.destroy();
    throw new HttpError(404, 'Conta não encontrada. Solicite a recuperação novamente.');
  }
...
async function changeAdminPassword({
  ...
  if (perfil !== 'admin' || !account) {
    throw new HttpError(403, 'Apenas administradores podem alterar a senha por esta área.');
  }
...
async function confirmRegistrationCode({ verification_id, codigo }) {
  ...
  if (!cadastroPendente) {
    throw new HttpError(404, 'Cadastro pendente não encontrado. Solicite um novo código.');
  }
  ...
  if (cadastroPendente.tentativas >= maxAttempts) {
    await cadastroPendente.destroy();
    throw new HttpError(429, 'Limite de tentativas atingido. Refaça o cadastro para receber outro código.');
  }
  ...
  if (!cadastroPendente.codigo_hash) {
    throw new HttpError(400, 'Cadastro pendente sem código. Solicite um novo código.');
  }
...
async function ensureUserIsAvailable(payload, transaction) {
  ...
  if (existingProprietario) {
    throw new HttpError(409, 'Este e-mail ou CPF já está cadastrado como proprietário.');
  }
  ...
  if (existingAdministrador) {
    throw new HttpError(409, 'Este e-mail ou CPF já está cadastrado como administrador.');
  }
}
...
async function ensureOwnerIsAvailable(payload, transaction) {
  ...
  if (existingProprietario) {
    throw new HttpError(409, 'E-mail ou CPF/CNPJ já cadastrado.');
  }
  ...
  if (existingUsuario) {
    throw new HttpError(409, 'Este e-mail ou CPF já está cadastrado como usuário.');
  }
...
async function updateProfile({ account, perfil, telefone, fotoPerfilUrl }) {
  if (!['usuario', 'proprietario', 'admin'].includes(perfil)) {
    throw new HttpError(403, 'Este perfil não permite edição de dados pessoais.');
  }
...
async function deleteAccount({ account, perfil }) {
  if (!['usuario', 'proprietario'].includes(perfil)) {
    throw new HttpError(403, 'Este perfil não permite exclusão de conta.');
  }
