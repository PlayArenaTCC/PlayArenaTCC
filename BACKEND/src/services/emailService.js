...
async function sendVerificationEmail({ to, code, name }) {
  ...
  const subject = 'Codigo de verificacao PlayArena';
  const text = [
    `Ola, ${recipientName}.`,
    '',
    `Seu codigo de verificacao PlayArena e: ${code}`,
    '',
    'Se voce nao solicitou esse cadastro, ignore este e-mail.',
  ].join('\n');
  const html = `
    <div style="font-family: Arial, sans-serif; color: #111827; line-height: 1.5;">
      <h2 style="margin: 0 0 12px;">Codigo de verificacao PlayArena</h2>
  ...
  console.info(`[Email local] Enviando codigo PlayArena para ${to}: ${code}`);
...
async function sendWithGmail({ html, subject, text, to }) {
  try {
    ...
    console.info(`[Email Gmail] Enviado para ${to}. Message ID: ${info.messageId}`);
    ...
    console.error('[Email Gmail] Erro ao enviar:', error.message);
...
async function sendWithSMTP({ html, subject, text, to }) {
  try {
    ...
    console.info(`[Email SMTP] Enviado para ${to}. Message ID: ${info.messageId}`);
...
async function sendWithResend({ html, subject, text, to }) {
  ...
  if (typeof fetch !== 'function') {
    throw new Error('Esta versao do Node nao possui fetch global para enviar e-mail.');
  }
