...
function normalizeCep(value) {
  ...
  if (!/^\d{8}$/.test(digits)) {
    throw new HttpError(400, 'Informe um CEP válido com 8 dígitos.');
  }
...
async function fetchJson(url, options = {}) {
  ...
  try {
    ...
    if (error.name === 'AbortError') {
      throw new HttpError(504, 'A consulta de localização demorou demais. Tente novamente.');
    }

    throw new HttpError(502, 'Não foi possível consultar o serviço de localização.');
  } finally {
...
async function geocodeAddress(location) {
  ...
  if (!endereco || !numero || !cidade || !estado) {
    throw new HttpError(400, 'Informe CEP e número para localizar o espaço.');
  }
  ...
  if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) {
    throw new HttpError(422, 'Não foi possível localizar este CEP e número no mapa. Confira os dados informados.');
  }
...
async function resolveCourtLocation(location = {}) {
  ...
  if (!endereco) {
    throw new HttpError(400, 'Este CEP não possui logradouro. Informe o endereço completo.');
  }
