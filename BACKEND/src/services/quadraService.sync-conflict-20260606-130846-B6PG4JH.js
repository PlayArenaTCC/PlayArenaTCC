...
function buildExplicitScheduleRows(quadraId, schedules, basePrice) {
  ...
  return schedules.map((schedule) => {
    ...
    if (
      ...
      throw new HttpError(400, 'Informe horarios disponiveis validos para o espaco.');
    }
...
async function getCourt(id) {
  ...
  if (quadra.ativa !== true || !isApprovedDocumentation(quadra.documentacao_local)) {
    throw new HttpError(404, 'Quadra indisponivel ou aguardando aprovacao documental.');
  }
...
async function createCourt(auth, {
  ...
  if (!ownerId || !nome || !cep || !numero) {
    throw new HttpError(400, 'Informe proprietario, nome, CEP e numero da quadra.');
  }
  ...
  if (location.precisao !== 'exata' && !hasConfirmedCoordinates) {
    throw new HttpError(422, 'Confirme a posicao exata do marcador no mapa antes de cadastrar a quadra.');
  }
...
async function createAdminCourt(auth, {
  ...
  if (auth?.perfil !== 'admin') {
    throw new HttpError(403, 'Apenas administradores podem cadastrar espacos por esta area.');
  }
  ...
  if (!owner) {
    throw new HttpError(400, 'Selecione um proprietario valido para o espaco.');
  }

  if (!trimmedName || !trimmedAddress || !trimmedCity) {
    throw new HttpError(400, 'Informe nome, endereco e cidade do espaco.');
  }

  if (!Number.isFinite(hourlyPrice) || hourlyPrice < 0) {
    throw new HttpError(400, 'Informe um valor valido para o espaco.');
  }
...
async function updateCourt(auth, id, body) {
  ...
  if (body.ativa === true && auth.perfil !== 'admin') {
    ...
    if (!isApprovedDocumentation(documentacao)) {
      throw new HttpError(400, 'A quadra so pode ser ativada apos aprovacao documental.');
    }
  }
...
async function setCourtTemporaryDeactivation(auth, id, {
  ...
  if (auth?.perfil !== 'admin') {
    throw new HttpError(403, 'Apenas administradores podem desativar espacos por periodo.');
  }
  ...
  if (!start || !end) {
    throw new HttpError(400, 'Informe data e hora de inicio e fim da desativacao.');
  }

  if (end <= start) {
    throw new HttpError(400, 'O fim da desativacao deve ser posterior ao inicio.');
  }

  if (end <= new Date()) {
    throw new HttpError(400, 'O fim da desativacao deve estar no futuro.');
  }
...
async function clearCourtTemporaryDeactivation(auth, id) {
  ...
  if (auth?.perfil !== 'admin') {
    throw new HttpError(403, 'Apenas administradores podem reativar espacos.');
  }
...
async function listSchedules(courtId, data) {
  ...
  if (quadra.ativa !== true || !isApprovedDocumentation(quadra.documentacao_local)) {
    throw new HttpError(404, 'Quadra indisponivel para reservas.');
  }
