...
async function sendVerificationSms({ phoneE164, code }) {
  ...
  const message = `Seu codigo de verificacao PlayArena e: ${code}`;

  if (provider === 'twilio_verify') {
    throw new Error('Twilio Verify gera e envia o codigo diretamente. Use startProviderVerification.');
  }
  ...
  console.info(`[SMS local] Enviando codigo PlayArena para ${phoneE164}: ${code}`);
...
async function sendWithZenvia({ phoneE164, message }) {
  ...
  if (typeof fetch !== 'function') {
    throw new Error('Esta versao do Node nao possui fetch global para enviar SMS.');
  }
...
async function twilioVerifyRequest(path, body) {
  if (typeof fetch !== 'function') {
    throw new Error('Esta versao do Node nao possui fetch global para chamar a Twilio.');
  }
