...
const documentLabels = {
  ...
  autorizacao_gerenciamento: 'Autorizacao do proprietario',
}
...
=======
  async function lookupCep(cep) {
    ...
    if (digits.length !== 8) {
      setLocationError('Informe um CEP valido com 8 digitos.')
      return false
    }
    ...
    try {
      ...
      setLocationError(
        localizacao.endereco
          ? ''
          : 'Este CEP nao possui logradouro automatico. Informe o endereco antes do numero.',
      )
      return true
    } catch (error) {
      if (cepRequestIdRef.current === requestId) {
  ...
  async function locateAddress() {
    if (!form.cep || !form.endereco || !form.numero || !form.cidade || !form.estado) {
      setLocationError('Informe um CEP valido e o numero do local para posicionar o espaco no mapa.')
      return false
    }
  ...
  function updateDocumentFile(key, file) {
    ...
    setForm((current) => ({
      ...
      documentacao: {
        ...current.documentacao,
        documentos: {
          ...current.documentacao.documentos,
          [key]: file,
        },
      },
    }))
  ...
  function validateDocumentation() {
    ...
    if (missingDocuments.length) {
      setDocumentationError('Envie todos os documentos obrigatorios antes de cadastrar o espaco.')
      return false
    }
  ...
  async function submit(event) {
    ...
    if (!hasCoordinates) {
      ...
      if (located.precisao !== 'exata') {
        setLocationError('A localizacao foi aproximada. Ajuste o marcador no mapa e confirme a posicao exata.')
        return
      }
    } else if (!form.localizacao_confirmada) {
      setLocationError('Confirme a posicao exata do marcador no mapa antes de continuar.')
      return
    }
    ...
    <>
      ...
      <section className="form-section wide-field document-step-card">
        <div className="form-section-header">
          <div>
            <strong>1. Validacao documental</strong>
            <small>Obrigatoria antes da liberacao do espaco esportivo</small>
          </div>
          <ShieldCheck size={20} />
        </div>
        ...
        {reusableDocumentation && (
          <div className="document-reuse-box">
            Documentacao aprovada encontrada para este endereco. Ela sera reutilizada automaticamente neste cadastro.
          </div>
        )}

        <label className="field wide-field">
          <span>Tipo de proprietario *</span>
          <select
            required
        ...
        <div className="document-upload-grid">
          {requiredDocumentKeys.map((key) => {
            ...
            return (
              <label className="document-upload-field" key={key}>
                <span>
                  <FileText size={15} />
                  {documentLabels[key]}
                </span>
                <input
                  ...
                  disabled={Boolean(reusableDocumentation)}
                  type="file"
                  onChange={(event) => updateDocumentFile(key, event.target.files?.[0] || null)}
                />
                <small>{selectedFile?.name || (reusableDocumentation ? 'Documento ja aprovado' : 'PDF, PNG, JPG ou WebP')}</small>
              </label>
            )
          })}
        </div>
      ...
      <section className="form-section wide-field">
        <div className="form-section-header">
          <div>
            <strong>2. Informacoes Basicas</strong>
      ...
      <section className="form-section wide-field">
        <div className="form-section-header">
          <div>
            <strong>3. Localizacao</strong>
            <small>Digite o CEP para preencher o endereco automaticamente e depois informe apenas o numero.</small>
          </div>
        </div>
        ...
        <label className="field">
          ...
          />
          <small className="field-help">{cepLoading ? 'Buscando CEP...' : 'Rua, bairro, cidade e UF serao preenchidos automaticamente.'}</small>
        </label>
        ...
        {geocoding && (
          <div className="location-progress-box">
            <MapPin size={17} />
            Localizando o CEP e numero no mapa...
          </div>
        )}
        ...
        {!geocoding && hasCoordinates && (
          <div className="location-picker-field">
            ...
            />
            <small>Arraste o marcador caso ele nao esteja exatamente na entrada do local.</small>
          </div>
        )}
        ...
        {!geocoding && hasCoordinates && form.localizacao_confirmada && (
          <div className="location-success-box">
            <MapPin size={17} />
            Posicao exata confirmada. Esta sera a localizacao exibida aos usuarios.
          </div>
        )}
        ...
        {!geocoding && hasCoordinates && !form.localizacao_confirmada && (
          <div className="location-confirmation-box">
            <span>
              <MapPin size={17} />
              Confira a bolinha no mapa e confirme a posicao exata do local.
            </span>
      ...
      <button className="primary-action wide-field" type="submit" disabled={loading || submitting || cepLoading || geocoding}>
        <Plus size={17} />
        {geocoding ? 'Localizando endereco...' : loading || submitting ? 'Salvando...' : form.campos.length > 1 ? `Cadastrar ${form.campos.length} campos` : 'Cadastrar quadra'}
>>>>>>> Stashed changes
      </button>
    </form>
  )
