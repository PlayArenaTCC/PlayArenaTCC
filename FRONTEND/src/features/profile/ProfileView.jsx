...
import { formatCpf, formatCpfCnpj, formatPhone, isValidPhone } from '../../utils/formatters'
...
export function ProfileView({
  ...
  const isUserProfile = profile.perfil === 'usuario'
  const canEditProfile = Boolean(onUpdateProfile) && ['usuario', 'proprietario', 'admin'].includes(profile.perfil)
  ...
  async function submit(event) {
    ...
    if (telefone && !isValidPhone(telefone)) {
      phoneInputRef.current?.setCustomValidity('Telefone deve ter DDD e 8 ou 9 dígitos.')
  ...
  async function submitPassword(event) {
    ...
    if (passwordForm.nova_senha !== passwordForm.confirmar_nova_senha) {
      showPasswordFieldError(formElement, 'confirmar_nova_senha', 'As senhas não conferem.')
      return
    }
  ...
  return (
    <section className={isUserProfile ? 'profile-screen user-profile-screen' : 'profile-screen'}>
      {canEditProfile && (
        <div className={isUserProfile ? 'settings-list profile-photo-card user-profile-photo-card' : 'settings-list profile-photo-card'}>
          ...
          {isUserProfile && (
            <div className="user-profile-mobile-heading">
              <strong>{nome}</strong>
              <span>{profile.email || 'Conta PlayArena'}</span>
            </div>
          )}
          <input
            ref={photoInputRef}
      ...
      {canChangePassword && (
        <form className="settings-list profile-password-card" onSubmit={submitPassword} noValidate>
          <h2>Alterar Senha</h2>
          <p>Defina uma nova senha para acessar o painel administrativo, sem validação por e-mail.</p>
          <label className="field profile-info-field">
            ...
            <input
              ...
              required
            />
            <small>Use 8 caracteres, letra maiúscula, número e caractere especial.</small>
          </label>
          <label className="field profile-info-field">
            <span>
              <LockKeyhole size={14} />
      ...
      <form className={isUserProfile ? 'profile-form user-profile-form' : 'profile-form'} onSubmit={submit}>
        <div className={isUserProfile ? 'settings-list profile-info-card user-profile-info-card' : 'settings-list profile-info-card'}>
          ...
          {profile.cpf_cnpj && (
            <label className="field profile-info-field">
              ...
              <input value={formatCpfCnpj(profile.cpf_cnpj)} readOnly />
            </label>
          )}
