...
export async function fetchRoleData(currentSession) {
  ...
  if (currentSession.usuario.perfil === 'admin') {
    ...
    const [
      ...
      apiRequest('/admin/avisos', { token: currentSession.token }),
...
export async function previewReservationTiming(token, payload) {
  const params = new URLSearchParams({
    data_reserva: payload.data_reserva || '',
    hora_inicio: payload.hora_inicio || '',
    hora_fim: payload.hora_fim || '',
  })
  const response = await apiRequest(`/reservas/pre-validar?${params}`, { token })
...
export async function validateReservationCode(token, reservaId, codigo) {
  const response = await apiRequest(`/reservas/${reservaId}/validar-codigo`, {
    method: 'PATCH',
    token,
    body: { codigo_reserva: codigo },
  })
...
function adminAccountBasePath(accountOrId) {
  return accountOrId?.tipo_conta === 'proprietario'
    ? `/admin/proprietarios/${accountOrId.id}`
    : `/admin/usuarios/${typeof accountOrId === 'object' ? accountOrId.id : accountOrId}`
}

export async function blockUserTemporarily(token, accountOrId, payload) {
  return apiRequest(`${adminAccountBasePath(accountOrId)}/bloqueio-temporario`, {
    method: 'PATCH',
    token,
    body: payload,
  })
...
export async function clearUserTemporaryBlock(token, accountOrId) {
  return apiRequest(`${adminAccountBasePath(accountOrId)}/bloqueio-temporario`, {
    method: 'DELETE',
    token,
  })
...
export async function banUser(token, accountOrId, motivo) {
  return apiRequest(adminAccountBasePath(accountOrId), {
    method: 'DELETE',
    token,
    body: { motivo },
  })
...
export async function unbanUser(token, accountOrId) {
  return apiRequest(`${adminAccountBasePath(accountOrId)}/desbanir`, {
    method: 'PATCH',
    token,
  })
}

export async function deleteManagedAccount(token, accountOrId, motivo) {
  return apiRequest(`${adminAccountBasePath(accountOrId)}/deletar-conta`, {
    method: 'DELETE',
    token,
    body: { motivo },
  })
...
export async function updateAdminAlertStatus(token, alertId, status) {
  const response = await apiRequest(`/admin/avisos/${alertId}/status`, {
    method: 'PATCH',
    token,
    body: { status },
  })
